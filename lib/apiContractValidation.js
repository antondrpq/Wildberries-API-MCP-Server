...
const assertLimitOffset = (body, { maxLimit = 1000, required = false } = {}) => {
  if (body.limit == null) {
    if (required) {
      fail('limit is required', 'limit');
    }
  } else {
    assertPositiveInteger(body.limit, 'limit');
    if (body.limit > maxLimit) {
      fail(`limit must be <= ${maxLimit}`, 'limit', { maxLimit });
    }
  }

  if (body.offset == null) {
    if (required) {
      fail('offset is required', 'offset');
    }
  } else {
...
const assertPeriodMaxDays = (period, maxDays, field = 'selectedPeriod') => {
  ...
  if (days > maxDays) {
    fail(`${field} must not exceed ${maxDays} days`, field, { maxDays });
  }
};
...
const validateSalesFunnelProducts = (body = {}) => {
  ...
  if (body.pastPeriod != null) {
    assertPeriod(body.pastPeriod, 'pastPeriod');
  }

  assertPeriodMaxDays(body.selectedPeriod, 365);

  if (body.nmIds != null) {
    assertArrayOfIntegers(body.nmIds, 'nmIds', 1000);
  }

  if (body.brandNames != null && !Array.isArray(body.brandNames)) {
    fail('brandNames must be an array', 'brandNames');
  }
  if (body.subjectIds != null) {
    assertArrayOfIntegers(body.subjectIds, 'subjectIds', 1000);
  }
  if (body.tagIds != null) {
    assertArrayOfIntegers(body.tagIds, 'tagIds', 1000);
  }
...
const validateSalesFunnelHistory = (body = {}) => {
  ...
  if (body.nmIds != null) {
    assertArrayOfIntegers(body.nmIds, 'nmIds', 20);
  }

  if (!AGGREGATION_LEVELS.has(body.aggregationLevel)) {
    fail('aggregationLevel must be day or week', 'aggregationLevel');
  }
};
...
const validateSalesFunnelGroupedHistory = (body = {}) => {
  ...
  const dimensions = [subjectIds.length, brandNames.length, tagIds.length];
  const nonEmptyDimensions = dimensions.filter(value => value > 0);
  const combinations = nonEmptyDimensions.length
    ? nonEmptyDimensions.reduce((acc, value) => acc * value, 1)
    : 0;

  if (combinations > 16) {
    fail('The selected subjects × brands × tags combinations must not exceed 16', 'filters', {
      combinations,
      maxCombinations: 16
    });
  }
...
const validateSearchReport = (body = {}) => {
  ...
  if (!SEARCH_POSITION_CLUSTERS.has(body.positionCluster)) {
    fail('positionCluster is required and must be a supported value', 'positionCluster');
  }

  if (!body.orderBy || typeof body.orderBy !== 'object' || Array.isArray(body.orderBy)) {
    fail('orderBy is required', 'orderBy');
  }
  if (typeof body.orderBy.field !== 'string' || !body.orderBy.field) {
    fail('orderBy.field is required', 'orderBy.field');
  }
  if (!['asc', 'desc'].includes(body.orderBy.mode)) {
    fail('orderBy.mode must be asc or desc', 'orderBy.mode');
  }
  ...
  if (body.includeSubstitutedSKUs === false && body.includeSearchTexts === false) {
    fail('includeSubstitutedSKUs and includeSearchTexts cannot both be false', 'includeSubstitutedSKUs/includeSearchTexts');
  }
};
...
const validateSearchProductSearchTexts = (body = {}) => {
  ...
  if (!SEARCH_TOP_ORDER_BY.has(body.topOrderBy)) {
    fail('topOrderBy must be one of openCard, addToCart, openToCart, orders, cartToOrder', 'topOrderBy');
  }
...
const validateSearchProductOrders = (body = {}) => {
  ...
  if (!Array.isArray(body.searchTexts)) {
    fail('searchTexts must be an array', 'searchTexts');
  }
  if (body.searchTexts.length > 100) {
    fail('searchTexts must contain at most 100 items', 'searchTexts', { maxLength: 100 });
  }
  if (body.searchTexts.some(value => typeof value !== 'string')) {
    fail('searchTexts must contain strings', 'searchTexts');
  }
};
...
const validateStocksProducts = (body = {}) => {
  ...
  if (body.currentPeriod != null) {
    assertPeriod(body.currentPeriod, 'currentPeriod');
  }
  if (body.availabilityFilters != null) {
    if (!Array.isArray(body.availabilityFilters)) {
      fail('availabilityFilters must be an array', 'availabilityFilters');
    }
    for (const value of body.availabilityFilters) {
      if (!STOCK_AVAILABILITY.has(value)) {
        fail(`Unsupported availability filter: ${value}`, 'availabilityFilters');
      }
    }
  }
...
const validateStocksProductsSizes = (body = {}) => {
  assertPlainObject(body);
  if (body.nmIDs != null) {
    assertArrayOfIntegers(body.nmIDs, 'nmIDs', 1000);
  }
...
const validateStocksOffices = (body = {}) => {
  assertPlainObject(body);
  if (body.currentPeriod != null) {
    assertPeriod(body.currentPeriod, 'currentPeriod');
  }
...
const validateFullstatsQuery = (query = {}) => {
  if (!query.ids) {
    fail('ids is required', 'ids');
  }
  const ids = String(query.ids).split(',').filter(Boolean);
  if (ids.length < 1 || ids.length > 50 || ids.some(value => !/^\d+$/.test(value))) {
    fail('ids must contain 1-50 comma-separated campaign IDs', 'ids');
  }

  if (!query.beginDate || !query.endDate) {
    fail('beginDate and endDate are required', 'beginDate/endDate');
  }
...
const asMiddleware = (validator) => (req, res, next) => {
  try {
    validator(req.body);
    next();
  } catch (error) {
...
const asQueryMiddleware = (validator) => (req, res, next) => {
  try {
    validator(req.query);
    next();
  } catch (error) {
