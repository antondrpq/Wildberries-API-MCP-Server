...
const errorHandler = (err, req, res, next) => {
  ...
  if (err.statusCode === 429 && err.retryAfter != null) {
    res.set('X-RateLimit-Retry', String(err.retryAfter));
  }
...
const callWbApi = async (url, method, data, headers) => {
  ...
  const retryAfter =
    response.headers['x-ratelimit-retry'] ??
    response.headers['X-RateLimit-Retry'];
...
// Statistics by search clusters
app.post('/api/adv/normquery/stats', async (req, res, next) => {
  try {
    const data = await callWbApi(
      `${API_URLS.ADVERT}/adv/v0/normquery/stats`,
      'POST',
      req.body,
...
// Statistics by search clusters
app.post('/api/adv/normquery/stats-v1', async (req, res, next) => {
  try {
    const data = await callWbApi(
      `${API_URLS.ADVERT}/adv/v1/normquery/stats`,
      'POST',
      req.body,
      { 'Authorization': req.apiKey }
    );
...
// WB API: GET /adv/v3/fullstats
app.get('/api/adv/fullstats', async (req, res, next) => {
  try {
    const data = await callWbApi(
      `${API_URLS.ADVERT}/adv/v3/fullstats`,
      'GET',
      req.query,
      { 'Authorization': req.apiKey }
    );
...
//
// Local routes are kept for backward compatibility.
// Internally they use the current WB Analytics v3 Sales Funnel API.

const normalizeSalesFunnelPeriod = (period) => {
  if (!period) {
    return undefined;
  }
...
const normalizeSalesFunnelProductsBody = (body = {}) => {
  const result = {
    selectedPeriod:
      body.selectedPeriod ||
      normalizeSalesFunnelPeriod(body.period) ||
      normalizeSalesFunnelPeriod(body.currentPeriod),

    pastPeriod:
      body.pastPeriod ||
      normalizeSalesFunnelPeriod(body.previousPeriod),
...
const normalizeSalesFunnelGroupedHistoryBody = (body = {}) => {
  const result = {
    selectedPeriod:
      body.selectedPeriod ||
      normalizeSalesFunnelPeriod(body.period) ||
...
// Local route preserved: POST /api/nm-report/detail
// WB API: POST /api/analytics/v3/sales-funnel/products
app.post('/api/nm-report/detail', async (req, res, next) => {
  try {
    const body = normalizeSalesFunnelProductsBody(req.body);

    const data = await callWbApi(
      `${API_URLS.ANALYTICS}/api/analytics/v3/sales-funnel/products`,
      'POST',
      body,
      { 'Authorization': req.apiKey }
    );
...
// Local route preserved: POST /api/nm-report/detail/history
// WB API: POST /api/analytics/v3/sales-funnel/products/history
app.post('/api/nm-report/detail/history', async (req, res, next) => {
  try {
    const body = normalizeSalesFunnelHistoryBody(req.body);

    const data = await callWbApi(
      `${API_URLS.ANALYTICS}/api/analytics/v3/sales-funnel/products/history`,
      'POST',
      body,
      { 'Authorization': req.apiKey }
    );
...
// Get product card statistics grouped by subjects, brands, and tags.
// Local route preserved: POST /api/nm-report/grouped/history
// WB API: POST /api/analytics/v3/sales-funnel/grouped/history
app.post('/api/nm-report/grouped/history', async (req, res, next) => {
  try {
    const body = normalizeSalesFunnelGroupedHistoryBody(req.body);

    const data = await callWbApi(
      `${API_URLS.ANALYTICS}/api/analytics/v3/sales-funnel/grouped/history`,
      'POST',
      body,
      { 'Authorization': req.apiKey }
    );
...
app.get('/api/nm-report/downloads/file/:downloadId', async (req, res, next) => {
  try {
    ...
    if (!uuidPattern.test(downloadId)) {
      return res.status(400).json({
        error: true,
        message: 'Invalid downloadId format'
      });
    }
    // This endpoint returns binary data (ZIP file)
    const response = await axios({
      method: 'GET',
      url: `${API_URLS.ANALYTICS}/api/v2/nm-report/downloads/file/${downloadId}`,
    ...
    res.set('Content-Disposition', `attachment; filename="report-${downloadId}.zip"`);
...
const evirmaUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (!allowedExt.test(file.originalname)) {
      return cb(new Error('РџРѕРґРґРµСЂР¶РёРІР°СЋС‚СЃСЏ С‚РѕР»СЊРєРѕ С„Р°Р№Р»С‹ .xlsx РёР»Рё .xls'));
    }
    cb(null, true);
  }
});
...
app.post('/api/evirma/import/keywords-report', (req, res, next) => {
  evirmaUpload.single('file')(req, res, async (uploadError) => {
    if (uploadError) {
      return res.status(400).json({
        error: true,
        message: `РћС€РёР±РєР° Р·Р°РіСЂСѓР·РєРё С„Р°Р№Р»Р°: ${uploadError.message}`
      });
    }
    ...
    if (!req.file) {
      return res.status(400).json({
        error: true,
        message: 'Р¤Р°Р№Р» РЅРµ РЅР°Р№РґРµРЅ. РћР¶РёРґР°РµС‚СЃСЏ multipart/form-data СЃ РїРѕР»РµРј "file".'
      });
    }
...
// Import the EVIRMA PRO "РЎС‚Р°С‚РёСЃС‚РёРєР° Р Рљ РїРѕ РґРЅСЏРј Рё Р·РѕРЅР°Рј РїРѕРєР°Р·РѕРІ" export
