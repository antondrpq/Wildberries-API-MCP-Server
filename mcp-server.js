...
const tools = [
  ...
  },
  {
    ...
    description: 'High-level financial summary for a period, built by automatically paginating POST /api/finance/v1/sales-reports/detailed server-side and aggregating the results. This endpoint has a strict WB rate limit, so each page retries with backoff (honoring the server\'s retry-after hint, up to 3 attempts) before giving up. Returns total revenue (forPay), WB commission, logistics, storage, acceptance, penalties, deductions, acquiring fees, and a per-nmId breakdown of the top items by revenue. Use this instead of wb_sales_report_detailed when you just need the totals rather than every raw row. Requires a token with the Finance category.',
    inputSchema: {
      type: 'object',
      properties: {
...
// The sales-reports/detailed endpoint has a noticeably strict WB rate limit
// (observed: repeated 429s even a minute apart). Retry with the server's
// own retryAfter hint before giving up, so wb_finance_summary/wb_business_summary
// don't fail on a single throttled page during a multi-page pull.
const FINANCE_RETRY_MAX_ATTEMPTS = Number(process.env.FINANCE_RETRY_MAX_ATTEMPTS || 2);
const FINANCE_RETRY_MIN_WAIT_MS = Number(process.env.FINANCE_RETRY_MIN_WAIT_MS || 5000);
const FINANCE_RETRY_MAX_WAIT_MS = Number(process.env.FINANCE_RETRY_MAX_WAIT_MS || 45000);

async function callWbWithRetry(url, method, data) {
  let lastError;
  for (let attempt = 1; attempt <= FINANCE_RETRY_MAX_ATTEMPTS; attempt += 1) {
    try {
      return await callWb(url, method, data);
    } catch (error) {
      lastError = error;
      if (error.statusCode !== 429 || attempt === FINANCE_RETRY_MAX_ATTEMPTS) throw error;

      const hintedMs = Number(error.retryAfter) * 1000;
      const waitMs = Math.min(
        Math.max(Number.isFinite(hintedMs) && hintedMs > 0 ? hintedMs : FINANCE_RETRY_MIN_WAIT_MS, FINANCE_RETRY_MIN_WAIT_MS),
        FINANCE_RETRY_MAX_WAIT_MS
      );
      await sleep(waitMs);
    }
  }
...
async function fetchAllSalesReportDetailed({ dateFrom, dateTo, period }) {
  ...
  for (let page = 0; page < FINANCE_SUMMARY_MAX_PAGES; page += 1) {
    const pageRows = await callWbWithRetry(`${API_URLS.FINANCE}/api/finance/v1/sales-reports/detailed`, 'POST', {
      dateFrom,
      dateTo,
      limit: 100000,
      rrdId,
